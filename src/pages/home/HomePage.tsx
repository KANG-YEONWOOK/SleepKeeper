...
async function fetchRecentSleepData(email: string, token: string|null): Promise<SleepData[]> {
  const response = await fetch('https://2r3hmaxnj4.execute-api.eu-north-1.amazonaws.com/data/threerecent', {
    method: 'POST',
    headers: {
...
export default function HomePage() {
  ...
  const [selectedDataType, setSelectedDataType] = useState<DataType>('sleep_time');
    ...
    datasets: [
      {
        ...
        data: sleepData.map(data => 
          selectedDataType === 'sleep_time' ? data.total_sleep_minutes : data.sleep_score
        ),
        backgroundColor: (context: any) => {
          ...
          gradient.addColorStop(1, selectedDataType === 'sleep_time' 
            ? 'rgba(81, 168, 222, 0.6)'
            : 'rgba(153, 102, 255, 0.6)'
          );
          return gradient;
  ...
  const chartOptions: ChartOptions<'bar'> = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: selectedDataType === 'sleep_time' ? '수면 시간 (분)' : '수면 점수',
        color: 'white',
        font: {
          size: 16
        }
      ...
      <main className="pt-16 px-4">
        <div className="flex flex-row items-center mt-6">
          ...
          <h1 className="text-xl font-bold">
            <span 
              className={selectedDataType === 'sleep_time' 
                ? 'text-[rgb(81,168,222)]' 
            ...
            <span className="text-white">님의 숙면을 지켜드려요!</span>
          </h1>
        </div>
              ...
              <div className="flex flex-col items-center">
                <div className="mb-6">
                  <ToggleSwitch
                    ...
                    onChange={() => setSelectedDataType(prev => 
